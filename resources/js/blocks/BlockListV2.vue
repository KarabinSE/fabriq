            ...
            >
                ...
                >
                    <li
                        ...
                        v-show="block.locale === activeLocale"
...
export default {
    ...
    computed: {
        ...
        noBlocks() {
            return this.localBlocks.filter(item => item.locale === this.activeLocale).length === 0
        },
    ...
    methods: {
        ...
        deleteBlock (index) {
            axios.delete('/api/blocks/' + this.localBlocks[index].id)
        ...
        blockTypeAdded (item) {
            ...
            this.$nextTick(() => {
                this.localBlocks.push(JSON.parse(JSON.stringify({ ...item, locale: this.activeLocale })))
            });
        ...
        async cloneBlock (block) {
            ...
            clonedBlock.content.name = 'Kopia av ' + block.content.name

            const { data } = await axios.post(`/api/pages/${this.$route.params.id}/revision/blocks`, { ...clonedBlock, locale: this.activeLocale})
            clonedBlock.id = data.data.id
